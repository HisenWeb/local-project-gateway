...
export function createReadonlyMcpServer() {
  const server = new McpServer(
    ...
    {
      instructions:
        "Readonly local project gateway. Tools may list whitelisted projects, list a project tree, read files and images from whitelisted project roots, and run predefined read-only diagnostic operations. OAuth protects the HTTP MCP endpoint. No remote file write, delete, arbitrary shell, git pull, npm install, service install, token, ssh key, or env file access."
    }
  );
